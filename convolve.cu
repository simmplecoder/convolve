...
void checked_call(std::size_t line, cudaError_t call_result)
{
    ...
    {
        std::cerr << "call failure at line " << line << ' ' << cudaGetErrorString(call_result)
                  << '\n';
        std::exit(-1);
    }
}
...
__device__ inline std::size_t to_flat_index(std::size_t rowidx, std::size_t columnidx,
                                            std::size_t column_count)
{
    return rowidx * column_count + columnidx;
}

__global__ void convolve(float* matrix, float* destination, std::size_t row_count,
                         std::size_t column_count)
{
    ...
    float sum = 0;
#pragma unroll
    for (int kernel_row_idx = -static_cast<int>(middle); kernel_row_idx <= static_cast<int>(middle);
         ++kernel_row_idx)
    {
        for (int kernel_column_idx = -static_cast<int>(middle);
             kernel_column_idx <= static_cast<int>(middle); ++kernel_column_idx)
        {
            const auto matrix_flat_index =
                to_flat_index(row + kernel_row_idx, column + kernel_column_idx, column_count);
...
int main()
{
    ...
    std::cout << generation_watch.elapsed_time<unsigned int, std::chrono::milliseconds>()
              << " milliseconds for matrix generation\n";
    shino::precise_stopwatch cuda_setup_watch;
    float* device_mat = nullptr;
    checked_call(__LINE__, cudaMalloc(&device_mat, mat.size() * sizeof(*mat.data())));
    checked_call(__LINE__, cudaMemcpy(device_mat, mat.data(), mat.size() * sizeof(*mat.data()),
                                      cudaMemcpyHostToDevice));
    float kernel_cell = 1.0f / 9.0f;
    std::cout << kernel_cell << '\n';
    std::vector<float> kernel_values(9, kernel_cell);
    checked_call(__LINE__, cudaMemcpyToSymbol(kernel, kernel_values.data(),
                                              kernel_values.size() * sizeof(*mat.data())));

    auto dest = std::vector<float>(sz * sz);
    float* device_dest = nullptr;
    checked_call(__LINE__, cudaMalloc(&device_dest, dest.size() * sizeof(*dest.data())));
    dim3 blockDim(32, 32);
    dim3 blockCount(std::ceil(sz / static_cast<double>(blockDim.x)),
                    std::ceil(sz / static_cast<double>(blockDim.y)));
    std::cout << cuda_setup_watch.elapsed_time<unsigned int, std::chrono::microseconds>()
    ...
    std::cout << kernel_watch.elapsed_time<unsigned int, std::chrono::microseconds>()
              << " microseconds for kernel execution\n";
    shino::precise_stopwatch copy_back_watch;
    checked_call(__LINE__, cudaMemcpy(dest.data(), device_dest, dest.size() * sizeof(*dest.data()),
                                      cudaMemcpyDeviceToHost));
    std::cout << copy_back_watch.elapsed_time<unsigned int, std::chrono::microseconds>()
